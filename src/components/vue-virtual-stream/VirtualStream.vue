...
export default {
  ...
  methods: {
    ...
    updateItemDimensions() {
      this.ready = false
      const sortedItems = this.$refs.items
        .filter(item => {
          return item.id !== null
        })
        .sort((a, b) => {
      ...
      this.totalHeight = !dimensionArray.length
        ? 0
        : dimensionArray.reduce((dimensionA, dimensionB) => {
    ...
    updateAllDimensions() {
      ...
      const newItemHeight = !this.$refs.newItems
        ? false
        : (this.$refs.newItems || []).reduce((a, b) => {
  ...
  created() {
    this.$on('resize-item', data => {
      if (this.watchResizes) {
        if (
          data.dimensions.w !== this.dimensions[data.id].width ||
          data.dimensions.h !== this.dimensions[data.id].height
        ) {
